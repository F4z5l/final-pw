...
ogRouter.get("/og/img", async (req, res) => {
  ...
  try {
    const upstream = await fetch(rawUrl, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; codexstudys-OG/1.0)",
...
ogRouter.get("/og/batch/:batchId", async (req, res) => {
  ...
  const frontendOrigin = process.env["FRONTEND_URL"] ?? "https://codexstudys.pages.dev";
  const apiOrigin = process.env["API_PUBLIC_URL"] ?? `${req.protocol}://${req.get("host")}`;
  const batchUrl = `${frontendOrigin}/batch/${batchId}`;

  let title = "codexstudys — JEE & NEET Video Player";
  ...
  try {
    const r = await fetch(`${PW_API}/v3/batches/${batchId}/details`, {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; codexstudys-OG/1.0)" },
    });
    if (r.ok) {
      const json: any = await r.json();
      const d = json?.data ?? {};
      if (d.name) {
        title = `${d.name} — codexstudys`;
        description = `Watch ${d.name} batch on codexstudys — JEE & NEET video lectures, live classes & DPP quizzes.`;
      }
...
<head>
  ...
  <meta property="og:site_name" content="codexstudys" />
