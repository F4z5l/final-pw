 ...
 * Handles both codexstudys and Vibrant Academy access via start-parameter prefix:
 *   codexstudys_<sessionId>  → checks @codexstudys channel
...
const APPS: Record<string, { channel: string; channelUrl: string; label: string }> = {
  codexstudys: {
    channel:    process.env.TELEGRAM_CHANNEL ?? "@codexstudys",
    channelUrl: "https://t.me/codexstudys",
    label:      "codexstudys",
  },
  ss: {
...
router.post("/bot/webhook", async (req, res) => {
  ...
  try {
    ...
    if (text.startsWith("/start")) {
      ...
      // Parse prefix: "codexstudys_<uuid>" or "ss_<uuid>"
